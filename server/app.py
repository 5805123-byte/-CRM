...
def ensure_schema():
    ...
    try:
        if not con.execute("SELECT 1 FROM seed_flags WHERE name='campaign_merge_v3'").fetchone():
            names = [r[0] for r in con.execute("SELECT DISTINCT TRIM(category) FROM donations WHERE COALESCE(category,'')<>''")]
            names += [r[0] for r in con.execute("SELECT DISTINCT TRIM(category) FROM pledges WHERE COALESCE(category,'')<>''")]
            names += [r[0] for r in con.execute("SELECT name FROM campaigns")]
            for nm in set(names):
                k = re.sub(r'[\s"\u05f4\'’]', '', nm)
                # גם "קמחא דסוכוח" — שגיאת הקלדה (ח במקום ת)
                if k.startswith('קמחאדסוכ') and nm != 'סוכות תשפ"ז':
                    campaign_merge(con, nm, 'סוכות תשפ"ז')
            con.execute("INSERT INTO seed_flags(name) VALUES('campaign_merge_v3')")
            con.commit()
    except Exception as e:
        ...
        if not con.execute("SELECT 1 FROM seed_flags WHERE name='recon_banquest_v1'").fetchone() and os.path.exists(bp):
            ...
            for d in donors:
                if d['english']:
                    ...
                    # שומרים גם את השם הפרטי: שם משפחה לבדו אינו מספיק —
                    # שני הרשקוביץ שונים אינם אותו תורם (מאיר).
                    if toks: bylast.setdefault(_ne(toks[-1]), []).append((d['id'], _ne(toks[0])))
            ...
            for x in json.load(open(bp, encoding='utf-8')):
                ...
                if not did:
                    ...
                    # התאמה לפי שם משפחה בלבד מותרת רק אם גם השם הפרטי
                    # מתאים (או לפחות האות הראשונה שלו). אחרת החיוב נשאר
                    # לא־משויך ומופיע ברשימת "לא זוהה כרטיס" לאישור.
                    if fn:
                        ok = [i for i, f in cand if f and (f == fn or f[:1] == fn[:1])]
                        if len(ok) == 1: did = ok[0]
                    elif len(cand) == 1:
                        did = cand[0][0]
                con.execute("""INSERT OR IGNORE INTO recon(tid,first,last,amount,date,addr,city,state,zip,phone,email,recurring,donor_id,category,processed,source,status)
    ...
    try:
        ...
        # מאיר: "חשין י\"ט נחמן ככה קוראים לו" — הגרשיים מבדילים אותו מחשין
        # נחמן האחר שברשימה. "ישי דוד" נרשם אצלנו הפוך מהקובץ, ומתיישר אליו.
        _same = [('חשין נחמן ברי"ט', 'חשין י"ט נחמן'),
                 ('חשין יט נחמן', 'חשין י"ט נחמן'),
                 ('ישי דוד', 'דוד ישי')]
        _merged = 0
    ...
    # מאיר: "זה קוויטל של הערשל פרידמן… כמו כולם עם פסיקים, בין כל משפחה תתחיל
    # שורה חדשה" · "תסדר את זה בלי ב'… קודם תמיד הבעל ואח"כ האשה, ואז הילדים
    # בן האמא שלהם. הילדים הלא נשואים כתובים למעלה". הקוויטל המלא מקובץ
    # ה-Word שלו: הוא ואשתו והילדים הרווקים, ואז כל משפחה נשואה בשורה משלה.
    try:
        if not con.execute("SELECT 1 FROM seed_flags WHERE name='friedman_hershel_kv_v2'").fetchone():
            hf = con.execute("SELECT id FROM donors WHERE last='פרידמן' AND first='הרשל'").fetchone()
            if hf:
                kv = '\n'.join([
                    "צבי חיים בן צפורה וזוג' שיינדל ברכה בת חנה, מרים מאליא בת שיינדל ברכה, יחזקאל שרגא בן שיינדל ברכה, פערל בת שיינדל ברכה",
                    "אהרן בן שיינדל ברכה וזוג' פערל בת שרה ליפשא, אברהם בן פערל, ישעיה בן פערל, אליעזר בן פערל, מרדכי בן פערל, יעקב בן פערל, שלום נתן נטע בן פערל, יטל בת פערל, חיה בת פערל, ישכר בעריש בן פערל, צפורה בת פערל, יוסף בנימין בן פערל, שמואל דוד בן פערל",
                    "לוי יצחק בן שיינדל ברכה וזוג' רחל מרים בת רבקה, רוזא בלומא בת רחל מרים, אברהם בן רחל מרים, שרה העסא בת רחל מרים, שמואל ישעיה בן רחל מרים, אהרן בן רחל מרים, שלמה אליעזר בן רחל מרים, חנה בת רחל מרים, ברוך בן רחל מרים, אסתר בת רחל מרים",
                    "יואל בן שיינדל ברכה וזוג' שרה פיגא בת העניא, יחיאל בן שרה פיגא, אברהם בן שרה פיגא, טויבא בת שרה פיגא, ישעיה בן שרה פיגא, מרדכי בן שרה פיגא, רויזא בת שרה פיגא, צפורה בת שרה פיגא, חנה בת שרה פיגא",
                    "יצחק יואל בן רבקה וזוג' חינקא רחל בת שיינדל ברכה, חוה סימא בת חינקא רחל, אסתר גאלדא בת חינקא רחל, ישעיה בן חינקא רחל, אברהם בן חינקא רחל, יעקב יוסף בן חינקא רחל, יחזקאל שרגא בן חינקא רחל, צפורה בת חינקא רחל",
                    "משה בן שיינדל ברכה וזוג' יהודית בת העסא רחל הדס, אליעזר זאב בן יהודית, אברהם בן יהודית, לאה בת יהודית, חיה בת יהודית, חנה בת יהודית, צפורה בת יהודית, שרה בת יהודית",
                    "ישעיה בן שיינדל ברכה וזוג' גאלדא בת איידל, אברהם ירחמיאל בן גאלדא, יחזקאל שרגא בן גאלדא, שרה רבקה בת גאלדא, לוי יצחק בן גאלדא, ליבא גיטל בת גאלדא, משה בן גאלדא",
                    "אברהם בן שיינדל ברכה וזוג' גאלדא בת נחמה, רבקה בת גאלדא, חנה בת גאלדא, אסתר בילא בת גאלדא",
                    "נתן נטע דוב בן ברכה חיה וזוג' סימא בת שיינדל ברכה, אברהם בן סימא, יצחק יהודה בן סימא, חנה בת סימא",
                    "אשר זעליג בן חיה רחל וזוג' חיה בת שיינדל ברכה, ישעיה בן חיה, אסתר ניחא בת חיה",
                    'להצלחה ברו"ג בברהגו"נ ופרנס"ט וזווגים הגונים ולכוח"ט',
                    'צבי מנחם בן חנה לרפו"ש',
                ])
                old = con.execute("SELECT id FROM prayers WHERE donor_id=? AND text LIKE '%צבי חיים%' ORDER BY id", (hf['id'],)).fetchall()
                if old:
                    con.execute("UPDATE prayers SET text=? WHERE id=?", (kv, old[0]['id']))
                    for r in old[1:]:
                        con.execute("DELETE FROM prayers WHERE id=?", (r['id'],))
                else:
                    con.execute("INSERT INTO prayers(donor_id,name,text,tier) VALUES(?,?,?,?)", (hf['id'], 'הרשל פרידמן', kv, 'קוויטל_שבועי'))
                print('  פרידמן הרשל: הקוויטל המלא נכנס לכרטיס')
            con.execute("INSERT INTO seed_flags(name) VALUES('friedman_hershel_kv_v2')")
            con.commit()
    except Exception as e:
...
# הנוסח — במילים של מאיר
KV_GEN = ('האברכים של הכולל חצות מתפללים ומזכירים את השמות והבקשות האלו בכל פעם '
          'כשהכולל חצות הולכים בעת רצון של חצות הלילה לכותל המערבי וקבר רחל ובציון הרשב"י במירון '
          'ובעוד זמנים מיוחדים של עת רצון')
KV_DESC = {
    'קוויטל_101': 'שלשה אברכים צדיקים מתפללים ומזכירים את השמות והבקשות האלו בכל לילה '
                  'בעת רצון של חצות, ' + KV_GEN,
    'קוויטל_שבועי': KV_GEN,
    'קוויטל_כללי': KV_GEN,
...
def donor_kvpage(con, did):
    ...
    if avs or tier == 'יששכר_זבולון':
        ...
        lines.append(('בנוסף לאברכים הלומדים ומתפללים בחצות לילה לזכותו, ' if len(avs) > 1
...
_CERT_CFG = {
    # מאיר: "חתוך פה למטה" — הטקסט ירד לתוך ציור הבניין שבתחתית
    # הבלאנק. הציור מתחיל ב-87.4% מגובה הדף (נמדד על התמונה עצמה),
    # ולכן תיבת הטקסט נעצרת ב-86.5% ומשאירה רווח.
    'parnes':    {'bg': 'letterhead.jpg',        'box': (0.25, 0.095, 0.95, 0.865)},
...
_CERT_SMALL = re.compile(
    r'^(בן|בת|בר|ב["\'״׳]ר|ובן|ובת|ז["\'״׳]ל|[ושה]?יב[א-ת]{1,5}["\'״׳][אט]|[א-ת]{1,6}["״][א-ת]{1,3}|ה["\'״׳]ה|ר|רב|ע["\'״׳]ה|ע["\'״׳]י|ע["\'״׳]ש|זצ["\'״׳]ל|זצוק["\'״׳]?ל|הי["\'״׳]ד|'
    r'נ["\'״׳]י|שליט["\'״׳]א|שיחי["\'״׳]?|שיחיו|תחי["\'״׳]?|ותחי["\'״׳]?|'
    r'הר["\'״׳]ר|הרר|הרב|רבי|ר["\'״׳]|מוה["\'״׳]ר|הרה["\'״׳][גחצקי]|הגה["\'״׳]צ|מרן|'
...
# ============================================================
#  כללי העיצוב הקבועים של התעודות — מאיר: "תעשה לי שיהיה נוסחה
#  קבועה עם כללים מראש בלי שאצטרך כל פעם להסביר לך מחדש".
#  כל שינוי עתידי בגדלים נעשה כאן, ולא בפיזור על פני הקוד.
#
#  1. סדר הגדלים, מהגדול לקטן:
#       א. השמות שמתפללים עליהם          — הגדולים ביותר
#       ב. "לעילוי נשמת" / "לזכות" ודומיהם — צמוד להם בגודל
#       ג. הבקשה שאחרי השמות              — בגופן של שורת הנוסח
#       ד. בן / בת / ע"ה / זצ"ל / זצוק"ל / רבי / מרת — הקטנים ביותר
#  2. כל שורות השמות בגודל אחיד. שורה ארוכה אינה מתכווצת לבדה —
#     כולן מתכווצות יחד, אחרת שם אחד יוצא ענק והשני ננסי.
#  3. הבקשה ("לזכות פלוני…") יורדת לשורה משלה ונשברת לפי הרוחב,
#     ולעולם אינה נדחסת לשורת השם.
#  4. התאריך ושורת הנוסח נצמדים לראש הדף תמיד, כדי שיישאר מקום
#     להקדשות ארוכות.
#  5. איפה שכתוב "לעילוי נשמת" או "לרפואת" — המילה "לזכות" יורדת,
#     גם מ"יהיו ויעמדו לזכות" וגם מסוף נוסח הקפה/הבוקר.
#  6. אין כותרת "פרנס יום" בתעודות הקפה והבוקר.
#  7. הגדלים מתאימים את עצמם לכמות הטקסט, תמיד לפי היחסים האלה.
# ============================================================
_REQ_R = .25        # הבקשה חוזרת לגודל של "יהי רצון" (1.0 מתוך 4.0)
_LEAD_R = .5        # "לעילוי נשמת" — חצי מהשם כפי שהוא נדפס בפועל
# בתעודת הקפה/הבוקר היחסים אחרים, לפי מה שמאיר ביקש:
#   "לעילוי נשמת" פי שניים מהגודל הקודם (חצי -> שלם),
#   ו"לזכות וכו'" בגופן של "ארוחת בוקר היום נתרמה" (1.18 מתוך 2.5).
# מאיר (אלול תשפ"ו): "לעילוי נשמת למה כזה גודל כמו השמות? זה אמור להיות
# מודגש אבל לא בגודל הזה" — חוזר ליחס של תעודת הפרנס: 40 מתוך 70 מהשם.
_LEAD_R_C = 40.0 / 70.0
_REQ_R_C = 1.18 / 2.5
...
_CERT_LEAD = re.compile(r'^(לע[יו]?לוי|נשמת|לע["\'״׳]?נ|לזכר|לזכות|לרפואת|לרפואה|לרפו["\'״׳]?ש|'
                        r'להצלחת|להצלחה|לישועת|לישועה|לפרנסת|לפרנסה|לזרע|לזיווג|'
                        r'לברכת|לברכה|לכבוד|לרגל)$')
# רק פתיח שבא *לפני שם* פותח הקדשה. ההבדל קיים בלשון עצמה: "לברכת" /
# "לרפואת" (סמיכות) באות לפני שם, ואילו "לברכה" / "לרפואה" הן הברכה
# עצמה שאחרי השם. מאיר: "פה אתה לא עשית לפי הכללים בכלל" — אצל
# "יהושע בן סימי וכל משפחתו לברכה והצלחה" נחשבה "לברכה" לפתיח, ולכן
# השם סווג כשם תורם והברכה סווגה כשם וקיבלה אותיות ענק.
_CERT_DED = re.compile(r'^(לע[יו]?לוי|נשמת|לע["\'״׳]?נ|לזכר|לזכות|לרפואת|להצלחת|'
                       r'לישועת|לפרנסת|לברכת|לכבוד|לרגל)$')
    ...
    for idx, raw in enumerate(raws):
        ...
        if not _CERT_LEAD.match(ws[0]):
            for k in range(1, len(ws)):
                ...
                if _CERT_DED.match(ws[k]) and _lead_len(ws[k:]) < len(ws[k:]):
                    ded = k; break
...
def _cert_join1(rows):
    """מאיר: "המילים מאחרי לעילוי נשמת אותו גודל עד המילה זצוק\"ל, חוץ
    מהמילים בן רבי. הרה\"ק רבי יהיו אותה שורה. הבקשה בשורה נפרדת."

    כלומר: שורת השמות מכילה שמות בלבד, כולם באותו גודל. מילות הלוואי
    שנגררו לסופה ("...הרה\"ק", "...בן") יורדות אל שורת התואר שאחריהן
    ומצטרפות אליה — "הרה\"ק רבי", "בן רבי" — בגודל של זצוק\"ל.
    מילת לוואי בסוף השם האחרון, שאין אחריה תואר, נשארת במקומה."""
    out = []
    for txt, role in rows:
        ws = str(txt or '').split()
        if role == 'ttl' and out and out[-1][1] == 'names':
            prev = out[-1][0].split()
            take = []
            while len(prev) > 1 and _CERT_SMALL.match(prev[-1]):
                take.insert(0, prev.pop())
            if take:
                out[-1] = (' '.join(prev), 'names')
                ws = take + ws
        ...
        if role == 'req' and len(ws) == 1 and out and out[-1][1] == 'req':
            out[-1] = (out[-1][0] + ' ' + ws[0], 'req')
            continue
        out.append((' '.join(ws), role))
...
def _req_at(ws, start=0):
    ...
    if rel >= 0 and rel + 1 < len(ws):
        # מאיר: "יבדל"א זה כמו ז"ל, ר' זה תואר בקטן, פסח זה שם" — תואר
        # שלפני שם ההורה ("בת יבדל"א ר' פסח") אינו שם ההורה
        i = rel + 1
        while i < len(ws) - 1 and _CERT_SMALL.match(ws[i]):
            i += 1
        i += 1
...
def _cert_body(lead, ws):
    ...
    def emit(seg):
        ...
        while t < len(seg) - 1 and _CERT_TITLE.match(seg[t]):
            t += 1
        if t:
            out.append((' '.join(seg[:t]), 'ttl'))
        rest = seg[t:]
        # מאיר: "את המילים לזכות וכו' תעשה בגופן של המילים ארוחת בוקר
...
def cert_png(kind='parnes', date='', names='', dedic='', width=1000, fmt='png', roles='',
    ...
    if kind in ('coffee', 'breakfast'):
        ...
        # מאיר: "אם יש לעילוי נשמת או לרפואת אז מורידים את המילה לזכות" —
        # לא רק ב"יהיו ויעמדו לזכות" של פרנס לילה, גם כאן בסוף הנוסח
        if _NESHAMA.search(names or ''):
