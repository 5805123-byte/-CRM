...
def ensure_schema():
    ...
    # מאיר: "זה קוויטל של הערשל פרידמן, צריך לסדר את זה שזה לא יהיה בשורה אחרי
    # שורה אלא שיהיה כמו כולם עם פסיקים, בין כל משפחה תתחיל שורה חדשה" —
    # הקוויטל המלא מקובץ ה-Word שלו (11 משפחות), כל משפחה בשורה משלה.
    try:
        if not con.execute("SELECT 1 FROM seed_flags WHERE name='friedman_hershel_kv_v1'").fetchone():
            hf = con.execute("SELECT id FROM donors WHERE last='פרידמן' AND first='הרשל'").fetchone()
            if hf:
                kv = '\n'.join([
                    "צבי חיים בן צפורה וזוג' שיינדל ברכה בת חנה, ב' מרים מאליא, ב' יחזקאל שרגא, ב' פערל",
                    "ב' אהרן וזוג' פערל בת שרה ליפשא, ב' אברהם, ב' ישעיה, ב' אליעזר, ב' מרדכי, ב' יעקב, ב' שלום נתן נטע, ב' יטל, ב' חיה, ב' ישכר בעריש, ב' צפורה, ב' יוסף בנימין, ב' שמואל דוד",
                    "ב' לוי יצחק וזוג' רחל מרים בת רבקה, ב' רוזא בלומא, ב' אברהם, ב' שרה העסא, ב' שמואל ישעיה, ב' אהרן, ב' שלמה אליעזר, ב' חנה, ב' ברוך, ב' אסתר",
                    "ב' יואל וזוג' שרה פיגא בת העניא, ב' יחיאל, ב' אברהם, ב' טויבא, ב' ישעיה, ב' מרדכי, ב' רויזא, ב' צפורה, ב' חנה",
                    "ב' חינקא רחל ובעלה יצחק יואל בן רבקה, ב' חוה סימא, ב' אסתר גאלדא, ב' ישעיה, ב' אברהם, ב' יעקב יוסף, ב' יחזקאל שרגא, ב' צפורה",
                    "ב' משה וזוג' יהודית בת העסא רחל הדס, ב' אליעזר זאב, ב' אברהם, ב' לאה, ב' חיה, ב' חנה, ב' צפורה, ב' שרה",
                    "ב' ישעיה וזוג' גאלדא בת איידל, ב' אברהם ירחמיאל, ב' יחזקאל שרגא, ב' שרה רבקה, ב' לוי יצחק, ב' ליבא גיטל, ב' משה",
                    "ב' אברהם וזוג' גאלדא בת נחמה, ב' רבקה, ב' חנה, ב' אסתר בילא",
                    "ב' סימא ובעלה נתן נטע דוב בן ברכה חיה, ב' אברהם, ב' יצחק יהודה, ב' חנה",
                    "ב' חיה ובעלה אשר זעליג בן חיה רחל, ב' ישעיה, ב' אסתר ניחא",
            ...
            con.execute("INSERT INTO seed_flags(name) VALUES('friedman_hershel_kv_v1')")
            con.commit()
    except Exception as e:
