...
מה עוד נעשה כאן כדי שהמייל יגיע לתיבה ולא לספאם:
  ...
  · כל הודעה נשלחת גם כטקסט פשוט וגם כ-HTML. הודעה שיש בה HTML בלבד
    נחשדת מיד.
  · כותרת List-Unsubscribe — כבויה (LIST_UNSUB). מאיר: "יש בג'ימייל ביטול
    הרשמה, זה לא מוצא חן בעיני — זה לא רשימת סתם אנשים, זה אנשי קשר אישיים,
    תורמים של הכולל." ג'ימייל דורש את הכותרת רק ממי ששולח יותר מ-5,000
    הודעות ביום; בדיוור של מאות לתורמים אישיים היא לא חובה.
...
# קישור "ביטול הרשמה" שג'ימייל מציג ליד שם השולח — מגיע מכותרת
# List-Unsubscribe. מאיר ביקש להוריד אותו; מי שלא רוצה לקבל, כותב לנו.
LIST_UNSUB = False
...
# מאיר: "תוריד את התואר של האברך שליט\"א, ורק לפני השם שלו תעשה ר'
# במקום הר\"ר, ואחרי השם אל תכתוב עוד תואר."
AV_PRE, AV_POST = "ר' ", ''             # התואר של האברך במכתב
...
def personalize(text, who, html=False, d='rtl'):
    ...
    def sub(m):
        ...
        if key in ('אברך', 'אברכים', 'avreich'):
            ...
            if not avs:
                return ''
            # התואר בגופן רגיל, השם עצמו מודגש. מאיר ביקש קודם את התואר
            # המלא, ואחר כך: "תוריד את התואר של האברך שליט\"א, ורק לפני
            # השם שלו תעשה ר' במקום הר\"ר" — ראה AV_PRE / AV_POST.
