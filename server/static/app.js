...
async function openIzSlip(av){
  ...
  // דף — מסרבל ובלגן. שיהיו כל השותפים באותו דף, מחולק, ורק לוגו אחד."
  const url=()=>{const p=new URLSearchParams();
    p.set('av',rows[0].avreich_p||rows[0].avreich); p.set('avt',rows[0].av_t||"ר'");
    rows.forEach(r=>{p.append('donor',r.donor_p||r.donor);
      p.append('dnt',r.donor_t===''?'':(r.donor_t||"ר'")); p.append('names',r.names||'');});
    return '/izslip.png?'+p.toString();};
  rs.innerHTML=`<button class="x" id="rx">✕</button><h2>🕯️ פתק קוויטל — ${esc(av)}</h2>
    <div class="izslipbox">
      <div class="hintxt">${rows.length>1?`<b>${rows.length} שותפים מחזיקים את האברך — כולם על אותו דף, מחולק בקו מפריד.</b><br>`:''}
        ${rows.map(r=>'זבולון: <b>'+esc(r.donor_t||'')+' '+esc(r.donor_p||r.donor)+'</b>'
          +(r.names?'':' · אין עדיין שמות לקוויטל')).join('<br>')}</div>
      <img class="izslipimg" src="${esc(url())}" alt="">
      <div class="addrow"><button class="btn sm izslipcopy">📋 העתק תמונה (דף שלם)</button>
        <button class="btn sm ghost izslipopen">🖨️ הדפסה (חצי דף)</button></div>
      <div class="addrow">${rows.map((r,i)=>`<button class="btn sm ghost izsliptxt" data-i="${i}"${r.names?'':' disabled'}>📝 השמות של ${esc(r.donor_p||r.donor)}</button>`).join('')}</div>
    </div>`;
  remov.classList.add('show');
  document.getElementById('rx').onclick=()=>remov.classList.remove('show');
...
// מאיר: "אני רוצה פשוט — תואר לפני השם, שם מלא מודגש, תואר אחרי השם,
// וקוויטל מודגש, ואם יש אברך שלומד בשבילו הוא מדגיש את השם שלו. וזהו."
const MLVARS=[['{{תואר}}','ה"ה — לפני השם'],
...
// מאיר: "אני רוצה להוסיף מתוך רשימה יותר ערוכה, בצורה יפה וברורה ובגופן
// גדול ונורמלי, שיהיה לי קל לסנן". לכן הבחירה נעשית מתוך רשימה מלאה עם
// תיבות סימון — מסננים למעלה, מסמנים למטה, ורואים בכל רגע מי נבחר.
function mlFiltered(){
  const q=mlQ.trim();
  let l=DB.slice();
  ...
  // מאיר: "לזמנים מיוחדים אותו אימייל בדיוק של מזדמנים — שניהם ביחד"
  else if(mlGrp==='occ+zt') l=l.filter(d=>hasOccKv(d)||(d.tier||'').trim()==='קוויטל_זמנים');
...
const MLTPL=[{
  ...
  body:[
   ...
   // מאיר: "אני רוצה שכל האימיילים ששולחים ליששכר זבולון, גם של יחיד וגם
   // יותר מאחד, שהשורה עם השם של האברך או האברכים יהיה בשורה נפרדת —
  ...
  // אחד רגיל ועוד אחד למי שיש יותר מאברך אחד" — זה השני. אותו נוסח
  // שאישר, ברבים, ועם {{מספר אברכים}} משולב במקומות שבהם המספר עצמו
  // הוא העניין. מאיר: "אם יש לתורם כמה אברכים שהוא מחזיק — שאכתוב '3
  ...
  // שמתפללים עליהם בחצות... שיידע שהשמות והבקשות האלו נאמרים באופן
  // אישי מכל הלב". אותו שלד שאישר ביששכר־זבולון — רק שבמקום אברך אחד
  ...
  body:[
   ...
   // הקוויטל שלו. להדגיש מאוד את הכותל וקבר רחל אבל בלי התחייבות שזה
  ...
  body:[
   ...
   // מאיר, במילים שלו: "בזכות התרומה הקבועה שלך אתה שותף פעיל ויש לך חלק
   // וזכות מיוחדת בקול התורה שנשמע ע"י למעלה ממאה אברכים צדיקים בכל
   // לילה בחצות. בזמן הקדוש והמסוגל המיוחד הזה שלשה אברכים נבחרים
   ...
   // מאיר: "זה חוזר על עצמו באותה שורה, תכניס את הכל בשורה אחת או שניים
  ...
  // הזדמנות ועת רצון כמו כשיש נסיעה לכותל המערבי או קבר רחל בחצות
  // לילה, או מירון אצל רבי שמעון... לא להדגיש את אומן." דרגת
  ...
  // שהשמות שלהם בקוויטל ותעשה כותרת אחרת בשבילם." בלי שורת הקוויטל
  ...
  body:[
   ...
   // מאיר: "אין צורך לפתיח שכזה כי הוא כבר תורם — להתחיל בלשון שהוא כבר
   ...
   '{{קו}}',
   'I deeply appreciate your support. Kollel Chatzos stands because of people like you, and every hour of it is written to your name.',
   'As every year, I will be zoche to be in Uman on Rosh Hashanah, at the holy tziyun of Rabbeinu Nachman of Breslov zy"a — you are welcome to send me your names, and I will daven for you there in Uman, for every name and every request, especially on Erev Rosh Hashanah, which is the most auspicious time of all to daven there.',
   'כתיבה וחתימה טובה\nשנה טובה ומתוקה\nשנה של בריאות, שמחה, אושר ועושר, ונחת וכל טוב אמן',
   'Rabbi Yehoshua Meir Deutsch\nRosh Kollel Chatzos, Eretz Yisroel',
   '{{קו}}',
...
function renderMailSend(){
  ...
  view.innerHTML=`<div class="addrow" style="margin:0 2px 8px"><button class="btn sm ghost" id="ml_back">← חזרה ליומן המיילים</button></div>
    ...
    <div class="sec">
      ...
      <div class="hintxt">לחיצה על סימון מכניסה אותו למכתב. חמישה סימונים, וזהו:<br>
        ...
        <b>{{אברך}}</b> — האברך שלומד עבורו: <i>ר'</i> <b>יעקב יוסף אנשין</b>.
  ...
  document.getElementById('ml_prev').onclick=async()=>{
    ...
    o.innerHTML=`<div class="sec mlprev"><div class="rbtitle">👁️ תצוגה מקדימה — עדיין לא נשלח כלום</div>
      ...
      <details class="mlplain"><summary>כך זה ייראה במייל שאינו תומך בעיצוב</summary>
        <pre class="mlsample">${esc(r.sample||'')}</pre></details>
...
function campOwes(cat){
  ...
  DB.forEach(d=>(d.pledges||[]).forEach(p=>{
    ...
    out.push({id:d.id,pid:p.id,name:(d.last+' '+d.first).trim(),amt:amtNum(p.amount),
      cur:(String(p.cur||'').trim()==='₪'?'₪':(String(p.cur||'').trim()==='$'?'$':curSym(d))),phone:splitPhones(d.phone)[0]||'',note:String(p.note||'').trim()});}));
  return out.sort((a,b)=>a.name.localeCompare(b.name,'he'));   // לפי א"ב של שם המשפחה
}
  ...
  box.innerHTML=`<div class="camphd"><h3>📊 שלושתם ביחד</h3>
    ...
    <div class="shtbl">
      ...
      ${shown.map(([x,i])=>`<div class="cshr" data-i="${i}">
        <span class="cshn">${x.donor_id?`<a class="avhold" data-did="${x.donor_id}">${esc(x.card_name||x.name)}</a>`:esc(x.name)}<button class="cshlnk" data-i="${i}" title="${esc(x.name)} — שייך לתורם אחר">🔗</button>${(x.names||[]).length>1?`<small class="cshsrc" title="שני שמות מהרשימות ששויכו לכרטיס הזה">ברשימה: ${esc(x.names.join(' + '))}</small>`:''}</span>
  ...
  box.querySelectorAll('.cshcell').forEach(cell=>cell.onclick=()=>{
    ...
    fm.querySelector('.cf_go').onclick=async()=>{
      ...
      if(!did){   // אין עדיין כרטיס לשם הזה — נפתח כרטיס, בלי שאלות
        const rr=await api('POST','/api/campaigns/link',{name:x.name,create:1,last:x.last,first:x.first});
        if(!rr||!rr.ok){toast('לא הצלחתי לפתוח כרטיס');fm.querySelector('.cf_go').disabled=false;return;}
...
async function campCompareTable(box){
  if(!box)return;
  if(CAMPMODE==='all')return campSheet(box);
  ...
  const fuzzy=x=>x.how&&/דומה|אנגלית/.test(x.how);   // זוהה לפי צליל / שם באנגלית — לא לפי שם זהה
  // שבב אחד לכל רשימה — הכל בשורה אחת, בלי גלילה לרוחב
  const listChip=(x,c)=>{
    const v=x.vals[c.key]; if(!v)return '';
    let tag='';
    if(x.donor_id)tag=x.have[c.key]?'<span class="cmpok" title="כבר רשום בכרטיס">✓</span>'
      :`<button class="cmpin" data-k="${esc(c.key)}" data-n="${esc(x.name)}" title="הכנס לכרטיס">📥</button>`;
    return `<span class="chipv"><small>${esc(c.label)}</small><b>${f(v)}</b>${tag}</span>`;
  };
  const nowChip=x=>{
    if(!x.donor_id||!campSel)return '';
    let st='';
    if(x.now)st=`<span class="cmppaid">✅ ${(x.methods||[]).map(m=>chBadgeRaw(m)||esc(chLabel(m))).join(' ')||'נגבה'}</span>`;
    else if(x.pledge)st=`<span class="cmpowe">🔴 טרם נגבה${x.pledge.amount?(' · '+f(x.pledge.amount)):''}</span>`;
    return `<span class="chipv now"><small>${esc(campSel)}</small>${x.now?`<b>${f(x.now)}</b>`:'<span class="cmpmiss">—</span>'}${st}<button class="cmpadd" data-i="${rows.indexOf(x)}" title="הוסף תרומה / התחייבות">➕</button></span>`;
  };
  const acts=(x,i)=>{
    if(x.donor_id&&!fuzzy(x))return '';
    const sg=(x.sugg||[]).map(sg=>`<button class="btn sm ghost cmpsg" data-i="${i}" data-did="${sg.id}" title="${esc(sg.why)}">🔗 ${esc(sg.name)}${sg.eng?` <small>${esc(sg.eng)}</small>`:''}</button>`).join('');
    return `<div class="cmpact">${sg?`<span class="cmpmaybe">אולי:</span>${sg}`:''}
      ${x.donor_id?'':`<button class="btn sm ghost cmpnew" data-i="${i}">➕ כרטיס חדש</button>`}
  ...
  const linkTo=async(x,did,label)=>{
    if(!await uiConfirm('לשייך את "'+x.name+'" מהרשימה לכרטיס של '+label+'?'))return;
    const rr=await api('POST','/api/campaigns/link',{name:x.name,donor_id:did});
    if(!rr||!rr.ok){toast('לא שויך');return;}
    toast('שויך ✓'); refresh();};
  // "אולי: …" — שיוך בלחיצה אחת להצעה
  box.querySelectorAll('.cmpsg').forEach(b=>b.onclick=()=>{
    const x=rows[+b.dataset.i], od=DB.find(y=>y.id==+b.dataset.did);
    linkTo(x,+b.dataset.did,od?(od.last+' '+od.first):b.textContent.trim());});
  ...
  box.querySelectorAll('.cmplnk').forEach(b=>b.onclick=()=>{
    ...
    inp.oninput=()=>{const q=inp.value.trim(); if(!q){res.innerHTML='';return;}
      ...
      res.innerHTML=h.list.map(y=>`<div class="dpr" data-did="${y.id}">${esc(y.last)} ${esc(y.first)} <span style="color:var(--muted)">#${y.id}${y.english?(' · '+esc(y.english)):''}</span></div>`).join('')+hitsMoreHTML(h)||'<div class="dpr" style="color:var(--muted)">אין תוצאות</div>';
      res.querySelectorAll('.dpr[data-did]').forEach(el=>el.onclick=()=>{
        const od=DB.find(y=>y.id==+el.dataset.did);
        linkTo(x,+el.dataset.did,od?(od.last+' '+od.first):('#'+el.dataset.did));});};
  ...
  box.querySelectorAll('.cmpadd').forEach(b=>b.onclick=()=>{
    const x=rows[+b.dataset.i], rowEl=b.closest('.cmprow');
    const old=box.querySelector('.cmpform'); if(old)old.remove();
...
function campPledgeEditor(row,d,pl){
  if(!row)return;
  const old=row.nextElementSibling; if(old&&old.classList.contains('cpedbox')){old.remove();return;}
  view.querySelectorAll('.cpedbox').forEach(x=>x.remove());
  const box=document.createElement('div'); box.className='cpedbox';
  const cur=(String(pl.cur||'').trim()==='₪'?'₪':(String(pl.cur||'').trim()==='$'?'$':curSym(d)));
  const chan=(String(pl.note||'').match(/דרך:\s*([^·]+)/)||[])[1]||'';
  box.innerHTML=`<div class="cmpform cped"><div style="width:100%"><b>${esc((d.last+' '+d.first).trim())}</b> — התחייבות ל${esc(pl.category||'')}</div>
    <span class="amtgrp"><input class="cf_amt cpamt" inputmode="decimal" value="${esc(pl.amount||'')}" placeholder="סכום"><button type="button" class="cf_cur cpcur" data-v="${cur}" title="לחץ להחליף בין דולר לשקל">${cur}</button></span>
    <select class="chansel cpm">${channelOpts(chan.trim())}</select>
    <input type="date" class="cpdate" value="${todayStr()}">
    <input class="cpnote" value="${esc(String(pl.note||'').replace(/\s*·?\s*דרך:[^·]*/,'').trim())}" placeholder="📝 הערה — למשל: בתשלומים, 3 תשלומים" style="flex:1 1 100%">
  ...
  const val=()=>({amt:box.querySelector('.cpamt').value.trim(),ccy:box.querySelector('.cpcur').dataset.v||'$',
    m:box.querySelector('.cpm').value==='__new__'?'':box.querySelector('.cpm').value,
  ...
  box.querySelector('.cpsave').onclick=async()=>{
    ...
    toast('ההתחייבות תוקנה ✓'); await load(); renderCamp();
  };
  box.querySelector('.cpgot').onclick=async()=>{
    const v=val(); if(!amtNum(v.amt)){toast('צריך סכום');return;}
    box.querySelector('.cpgot').disabled=true;
    const rr=await api('POST','/api/donation',{donor_id:d.id,amount:v.amt,category:pl.category||'',method:v.m,date:v.date,note:v.note,cur:v.ccy});
    if(rr&&rr.merged)toast('החיוב הזה כבר נכנס מהאשראי — סווג ל'+(pl.category||'')+' בלי שורה כפולה');
    await putPledge({status:'נתן',amount:v.amt,cur:v.ccy});
    toast('נרשם כנגבה ✓'); await load(); renderCamp();
  };
  box.querySelector('.cpdel').onclick=async()=>{
    if(!confirm('למחוק את ההתחייבות של '+(d.last+' '+d.first).trim()+' ('+cur+Math.round(amtNum(pl.amount)).toLocaleString('en-US')+')?'))return;
    await api('DELETE','/api/pledge/'+pl.id);
    toast('ההתחייבות נמחקה'); await load(); renderCamp();
  };
...
function renderCamp(){
  ...
  // מאיר: "אני רוצה שהכל יהיה לפי אלף בית של שם המשפחה מסודר"
  grp.sort((a,b)=>a.main.name.localeCompare(b.main.name,'he'));
  view.innerHTML=`<div class="avbar noprint">
